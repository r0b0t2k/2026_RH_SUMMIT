...
- Extracts PDF text + embedded images
...
def extract_pdf_text_and_images(pdf_path: Path, img_dir: Path) -> tuple[str, list[dict[str, str]]]:
    if fitz is None:
        raise RuntimeError(
...
def build_bundle(talk_dir: Path, output_dir: Path) -> None:
    ...
    if pdf_files:
        for pdf_path in pdf_files:
            pdf_text, rows = extract_pdf_text_and_images(pdf_path, img_dir)
